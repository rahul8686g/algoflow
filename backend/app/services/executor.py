...
Executes workflow nodes using the broker API SDK
"""
    ...
    async with lock:
        async with async_session_maker() as db:
            ...
            try:
                ...
                if not client:
                    raise Exception("Broker API not configured")
...
async def _activate_price_alert(workflow: Workflow, trigger_node: dict, db: AsyncSession) -> dict:
    ...
    try:
        ...
        if not settings or not settings.openalgo_api_key:
            return {"status": "error", "message": "Broker API settings not configured"}
